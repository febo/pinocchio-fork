<p align="center">
 <img alt="pinocchio-token-2022" src="https://github.com/user-attachments/assets/4048fe96-9096-4441-85c3-5deffeb089a6" width="127" height="100"/>
</p>
<h3 align="center">
...
```rust
use pinocchio_token_2022::instructions::InitializeMint;
...
The `Batch` (instruction discriminator `255`) enables efficient CPI interaction with the Token-2022 program.
This is a new instruction that can execute a variable number of Token-2022 instructions in a single invocation
of the program. Therefore, the base CPI invoke units (currently `1000` CUs) are only consumed once,
instead of for each CPI instruction. This significantly improves the CUs required to perform multiple
Token-2022 instructions in a CPI context.
...
The helpers in this crate offer two ways to use a different token program during invoke. Instead of using `invoke` and `invoke_signed`, programs can use:
  ...
  - `invoke_with_program` and `invoke_signed_with_program`: these accept an additional `Address`
    parameter and validate that it is equal to the Token-2022 program address.
